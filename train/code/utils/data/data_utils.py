...
def create_dataset_split(current_dataset, raw_dataset, train_phase, tokenizer,
    ...
    def _addrole_masklabel_tokenize(source):
        ...
        targe  = source["response"] 
        label_value =  targe 

        sentence_value = f'''<s>[INST] {question}[/INST]'''
       
        conversation += sentence_value
        sentence_ids = tokenizer.encode(sentence_value, add_special_tokens=False)
        label = tokenizer.encode(label_value, add_special_tokens=False)
        labels += [IGNORE_INDEX] * len(sentence_ids) + label
