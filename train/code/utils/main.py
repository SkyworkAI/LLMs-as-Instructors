...
def parse_args():
    ...
    parser.add_argument("--show_loss_step", default=10, type=int, help = "Show the loss step")
    parser.add_argument("--max_new_tokens", default=1024, type=int, help = "Max number of output tokens")
    parser.add_argument("--fp16", default=False)
    parser.add_argument("--bp16", default=True)
...
def main():
    ...
    def evaluation(model, eval_dataloader):
        ...
        losses = losses / torch.tensor(len(eval_dataloader) + 1)
        model.train()
        try:
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in tqdm(enumerate(train_dataloader), total=len(train_dataloader), unit="batch"):
            ...
            if (step+1)%args.show_loss_step == 0:
                ...
                training_step_losses = []
            if args.output_dir is not None and (step+1 + epoch *len(train_dataloader))% 1000000 == 0:
                print_rank_0('saving the final model ...', args.global_rank)#It will overwrite the last epoch model
                model = convert_lora_to_linear_layer(model)

                if args.global_rank == 0:
                    save_hf_format(model, tokenizer, args, sub_folder= f"{step+1 + epoch *len(train_dataloader)}")
